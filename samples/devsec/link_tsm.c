					    ...
					    struct kvm *kvm, u32 tdi_id)
{
	...
	pci_tsm_tdi_constructor(pdev, &devsec_tdi->pci, kvm);
...
static int devsec_link_tsm_guest_req(struct pci_tdi *tdi,
				     enum pci_tsm_req_scope scope,
				     const void *req_in, size_t in_len,
				     void *req_out, size_t *out_len,
				     u64 *tsm_code)
{
	dev_dbg(pci_tsm_host(tdi->pdev), "%s\n", pci_name(tdi->pdev));
	print_hex_dump_debug("devsec req_in  ", DUMP_PREFIX_OFFSET, 16, 4,
			     req_in, min(in_len, 256u), true);
				 ...
				 const char *buf, size_t count)
{
	...
	rc = pci_tsm_guest_req(pdev, PCI_TSM_REQ_INFO, buf,
			       count, req_out, &out_len, &tsm_code);
	if (rc)
		return rc;
